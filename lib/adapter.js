...
module.exports = (function () {
  ...
  var adapter = {
    ...
    drop: function(connectionName, collectionName, relations, cb) {
      ...
      collection.destroy(criteria).exec(function(err, obj) {
        if(err) {
          ...
          adapter.scheduleCommit(connectionName);
          cb(null, obj);
        }
      })
    },
    ...
    // Schedule a commit according to the commit delay configuration
    // to avoid running multiple simultaneous commits
    scheduleCommit: function (connectionName) {
      var connection = connections[connectionName];
      var settings = connection.sailsSettings;
      var isScheduled = this.scheduledCommitTimeout;
      var delay = settings.commitDelay || 2000; 
      var adapter = this;
      
      if(!isScheduled) {
        var commit = function() {
          adapter.commit(connectionName);
        }
        adapter.scheduledCommitTimeout = setTimeout(commit, delay);
      }
    },

    commit: function (connectionName) {
      var client = connections[connectionName];
  
      // Allow new commits to run by clearing the timeout ID
      clearTimeout(this.scheduledCommitTimeout);
      this.scheduledCommitTimeout = false;

      client.commit(function(err, obj) {
        if(err) {
          sails.log.error('There was an error doing a Solr commit: ' + err);
        }
        else {
          sails.log.debug('Solr commit completed');
        }
      });
    },
